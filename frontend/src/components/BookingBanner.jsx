...
function BookingBanner({ hideDropdowns, hideReturning,  from, to , fromIataCode, toIataCode, tripType, tripClass, departureDate, returnDate }) {
    return (
        <div className='mx-auto text-white bg-[#454545] px-12 w-full grid grid-cols-1 md:grid-cols-1 lg:grid-cols-12 '>
            <div className='col-span-10'>
                
                <h1 className='flex flex-col text-8xl p-4 roboto-bold'>
                    ...
                    <span className='px-2 flex justify-between text-xl w-2/5'>{from}<div>{to}</div></span>
                </h1>
                <div className='border-2 mb-8 border-[#adadad] grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 w-2/5 rounded-lg p-2 whitespace-nowrap'>
                    ...
                    {hideReturning ? <div></div> : <BookingReturningDate returnDate={returnDate} />}
                </div>
            </div>
